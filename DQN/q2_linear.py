...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        h, w, c = state_shape
        self.s = tf.placeholder(shape=(None, h, w, c*config.state_history), dtype=tf.uint8) 
        self.a = tf.placeholder(shape=(None, ), dtype=tf.int32)
        self.r = tf.placeholder(shape=(None, ), dtype=tf.float32)
        self.sp = tf.placeholder(shape=(None, h, w, c*config.state_history), dtype=tf.uint8)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        with tf.variable_scope(q_scope):
            q_list = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
        with tf.variable_scope(target_q_scope):
            q_target_list = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)

        ops = []

        for q, q_target in zip(q_list, q_target_list):
            ops.append(tf.assign(q, q_target))
    ...
    def add_loss_op(self, q, target_q):
        ...
        q_cur = tf.reduce_sum(tf.one_hot(self.a, num_actions)* q, axis=1)
        self.loss = tf.reduce_mean(tf.squared_difference(q_samp, q_cur))
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_optimizer_op(self, scope):
        ...
        grad = adam.compute_gradients(self.loss, var)
        if self.config.grad_clip:
            grad = [(tf.clip_by_norm(var_grad, self.config.clip_val), var) for var_grad, var in grad if var_grad != None]
        self.train_op = adam.apply_gradients(grad)
        v_grad, v = zip(*grad)
        self.grad_norm = tf.global_norm(v_grad)
        ##############################################################
        ######################## END YOUR CODE #######################
